		...
		docs.forEach(function(ele, key, _array) {
			Object.keys(ele.ResponseTime).forEach(function(minute, key, _array) {
				Object.keys(ele.ResponseTime[minute]).forEach(function(second, key, _array) {
					...
					var tmp = new Date(year, month, day, hour, minute, second, 0);
...
var minute_avg = function(data ,done){
	...
	data.forEach(function(ele, key, _array) {
		if (minute == ele.x.getMinutes()){
			...
			var day = date.getUTCDate();
			var hour = date.getHours();
			var tmp = new Date(year, month, day, hour, ele.x.getMinutes(), 0, 0);
			tmp.setMinutes(date.getMinutes() - 1);
...
var hour_avg = function(data ,done){
	async.series([
			function(done){
				minute_avg(data, done);
			}
	],
	// optional callback
	function(err, result){
		...
		var avg = 0.0;
		var hour_count = 0;
		var hour = data[0].x.getHours();
		var arr = [];
		data.forEach(function(ele, key, _array) {
			if (hour == ele.x.getHours()){
				sum += ele.y;
				hour_count++;
			}else{
				var date = new Date(ele.x);
				...
				var month = date.getMonth();
				var day = date.getUTCDate();
				var tmp = new Date(year, month, day, ele.x.getHours(), 0, 0, 0);
				tmp.setHours(date.getHours() - 1);
				avg = sum / hour_count;
